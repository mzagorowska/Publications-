...
plot([0:0.01:tuningHorizon], 1e-5*setpoint.(0:0.01:tuningHorizon), lw=2, lc=:black, ls=:dash, leg=false)
...
for k in eachindex(tAll2)
    if ik <= 0
        plot!( tAll2[k], 1e-5 * y1All.(k, 1:length(tAll2[k])), lw=2, label=raw"β=" * string(β[ib, :]), lc=palette(:default)[ib+1])
    else
        plot!(tAll2[k], 1e-5 * y1All.(k, 1:length(tAll2[k])), lw=2, label=:none, lc=palette(:default)[ib+1])
    end
    ik += 1
end
plot!()
