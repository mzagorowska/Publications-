...
allVal = [0.04 1.0 ######For 1e-1 5e1  and constant
    #0.19424 0.9260255063387
    0.1961 0.46352550633869
    # 0.1861 0.4621455063387
    0.2021 0.23102550633869
    # 0.2031 0.11104550633869
    0.2231 0.11002550633869
    0.4681 0.05833550633869
    # 0.1001 0.0
]

# allVal = [0.0 1.0 ######For 1e-1 5e1  and step
# #0.0321 1.0
# 0.1831 0.7570255063387
# # 0.1831 0.7570255063387
# 0.2001 0.23016550633869
# # 0.1981 0.13002550633869
# 0.2071 0.13002550633869
# 0.2371 0.07702550633869
# # 0.999 0.00264
# ]

# allVal = [0.0 1.0 ######For 1e-1 5e1  and sine
# #0.0 1.0
# 0.0851 1.0
# # 0.0901 1.0
# 0.1991 0.8080255063387
# # 0.2301 0.10464550633868
# 0.2291 0.10465550633869
# 0.2781 0.0759155063387
# # 0.999 0.00208
# ]
...
#  p = plot([0:0.01:tuningHorizon],setpoint.(0:0.01:tuningHorizon).*1e-5,lw=2,lc=:black,ls=:dash,label="Setpoint")
p = plot() #####For control
plot!([0, 200], [1000, 1000], lc=:black, ls=:dash, lw=2, label=:none)
...
for i3 in eachrow(allVal)
    ...
    p = plot!([0:0.01:tuningHorizon], u1intp.(0:0.01:tuningHorizon), linetype=:steppre, lw=2, label=raw"β=" * string(β[ib, :])) #####Control for tuned
    ...
    #     if ik <=0 
    #     plot!(p,tAll2[k],1e-5*y1All.(k,1:length(tAll2[k])),lw=2,label=raw"β="*string(β[ib,:]),lc=palette(:default)[ib+1])
    #     else
    #         plot!(p,tAll2[k],1e-5*y1All.(k,1:length(tAll2[k])),lw=2,label=:none,lc=palette(:default)[ib+1])
    #     end
    #     ik+=1
    # end
...
# ik = 0
# for k in eachindex(tAll2)
#     # @show 1e-5*y1All.(k,1:length(tAll2[k]))
#     # @show tAll2
#     if ik <= 0
#         plot!( tAll2[k], 1e-5 * y1All.(k, 1:length(tAll2[k])), lw=2, label="Initial guess", lc=:black)
#     else
#         plot!(tAll2[k], 1e-5 * y1All.(k, 1:length(tAll2[k])), lw=2, label=:none, lc=:black)
#     end
#     ik += 1
# end

p = plot!([0:0.01:tuningHorizon], u1intp.(0:0.01:tuningHorizon), linetype=:steppre, ls=:dot, lw=2, lc=:red, label="Initial guess") #####Control
plot!([0, 200], [300, 300], lc=:black, ls=:dashdot, lw=2, label=:none)

initP = [1.5e2, 47.5] #####Manually tuned
...
p = plot!([0:0.01:tuningHorizon], u1intp.(0:0.01:tuningHorizon), linetype=:steppre, lw=2, lc=:black, label="Manual") #####Control
...
#     # @show tAll2
#     if ik <= 0
#         plot!( tAll2[k], 1e-5 * y1All.(k, 1:length(tAll2[k])), lw=2, label="Manual", lc=:black)
#     else
#         plot!(tAll2[k], 1e-5 * y1All.(k, 1:length(tAll2[k])), lw=2, label=:none, lc=:black)
#     end
#     ik += 1
...
# initPNomad = 1.0 ./ (ubnd .- lbnd) .* initP .- lbnd ./ (ubnd .- lbnd)
# resError, resOver, resOsc = perfIndicators(initPNomad,x0,1)

# ylims!(0.83, 1.03) ######For responses
